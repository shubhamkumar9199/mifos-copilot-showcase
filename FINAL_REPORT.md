...
**repositories:** [openMF/web-app](https://github.com/openMF/web-app) (the panel) and [openMF/mcp-mifosx](https://github.com/openMF/mcp-mifosx) (the gateway)
...
The whole design rests on one rule. **The assistant can propose a change to a financial record and can never make one.** When it wants to write, the reply stops, the officer is shown a card naming the client, the product and the amount in words they recognise, and nothing runs until a person presses approve.

The deliverables:

- **Copilot panel** — an Angular chat panel embedded in the web app, translated into all 13 locales, off by default and switched on per deployment
- **Copilot Gateway** — a new Java 21 service whose reasoning core carries no framework imports, so it can later be embedded inside Fineract rather than deployed beside it
- **Write pause** — writes suspend the response stream and resume only on an explicit human decision, on a separate endpoint
- **Credential passthrough** — the gateway holds no Fineract account of its own and acts as the officer, so existing permissions and the audit trail still apply
- **Default-deny tool manifest** — 12 tools declared in one file, 7 that read and 5 that write; anything else the model names is refused
- **Thinking output** — what the assistant did and what the model wrote while doing it, kept apart because only one of them is a record

## Architecture

```
  Officer's browser                Copilot Gateway                  Fineract
  ─────────────────                ───────────────                  ────────
  Angular panel      ──SSE──▶      guards: credential,      ──▶     the officer's own
  11 components                    backend origin, length           permissions decide
  13 locales                              │
       ▲                                  ▼
       │                           agent loop, max 6 rounds
       │                                  │
       │                                  ├──▶ model (Groq / OpenAI / on-prem Ollama)
       │                                  │
       │                                  └──▶ tool manifest, default deny
       │                                            │
       └────── card, awaiting a person ◀────── a write stops here
```
...
### 1. The panel: components, theming, feature flag, lazy loading
...
Three decisions from this period that held for the rest of the project:

- **Translations from the start, not after.** All 13 locales the web app supports were populated in the same PR as the strings, rather than left as an English placeholder to be filled in later.
- **Context detection.** The panel reads the router to know which client or loan is on screen, so nobody types a database id into a chat box.
- **An input sanitiser between the officer and the model**, before there was a model to send anything to.

### 2. The gateway

[#421](https://github.com/openMF/mcp-mifosx/pull/421)

The service that turns a sentence into banking operations. Java 21 and Spring Boot WebFlux, but the reasoning core imports no framework at all, which a test enforces rather than a convention: `FrameworkFreeCoreTest` fails the build if anything in `core` reaches for Spring. The Spring shell is five classes.
...
The write pause is built in here rather than bolted on. When the model asks for a write, the loop stops before executing anything, resolves the values that would actually be sent, reads back the client and product names so the card can use them, and emits the card **without a completion event**. The turn is suspended. Execution happens only through `POST /actions/{cardId}/decision`, and a card is taken rather than read, so it cannot be spent twice.
...
Cards are now built by the server from the parsed call and never from model prose, with labelled rows, money formatted as money, dates as dates, and the client and product named by reading them back from Fineract first. The receipt after a write names the account rather than reporting `Loan ID 1`.

### 5. Reading values from where they live

[#438](https://github.com/openMF/mcp-mifosx/pull/438)

Seven defects of a single shape: a value that should come from the product, the officer's credential or Fineract's own calendar was written as a literal instead. The worst was that every client was created in office 1 regardless of who created them. The office is now derived from the officer's own credential, and fails closed if it cannot be established.

### 6. Holding the assistant to the same rules as the form

[#439](https://github.com/openMF/mcp-mifosx/pull/439)

A reviewer created a client whose first name was `999999999999999`, and Fineract accepted it. The web app's own forms would have refused it, because the validation lived in the form rather than in the API.

Rather than invent a second set of rules, the manifest now carries the constraints the web app's validators already enforce, per parameter, and a value that fails them never reaches the bank. A broken pattern in the manifest is swallowed rather than thrown, so a typo cannot take a tool out of service.
...
`#446` corrected it to `displayName`, which is the filter that exists, and handles the casing on our side: a name typed in one case is capitalised, and a name carrying capitals of its own is left exactly as written, because `McDonald` rewritten as `Mcdonald` finds nobody.
...
It now reads `reasoning`, `reasoning_content` and `thinking` across providers, and handles models that leave `<think>` markers inline in the content. That last case is harder than it looks: deltas are fragments, `"<th"` then `"ink>"` is ordinary, and anything released early is on the officer's screen for good. Any tail that could still grow into a marker is held until the next chunk decides it. The tests feed the text one character at a time, because if it survives that it survives any chunking a provider can produce.

The panel shows one collapsed strip under each reply, carrying two things that are not the same kind of thing, which is discussed under Challenges below.

## Testing and CI

- **157 gateway tests.** Pure JUnit against the framework-free core, including `FrameworkFreeCoreTest`, which fails the build if the core acquires a framework import, and `RedactionPostureTest`, which pins what every tool masks so that adding a tool or dropping a mask fails the suite rather than passing quietly.
- **168 panel tests** under Jest.
- **Playwright end to end** in the web app's existing CI, which stands up a real Fineract in Docker. The suite turns the Copilot on itself by rewriting the deployment config as it is served, so it genuinely runs rather than skipping on a flag.
- Every change was driven against the community sandbox with a real model before it went up, and several of the defects below were found that way rather than by a test.
...
Instead the pause lives in the loop. A write tool is intercepted before execution, the arguments are normalised so the card and the eventual request cannot disagree, and the stream ends without a completion event. There is no path from a model's output to a Fineract write that does not pass through a human pressing approve, because the only code that executes a write is on the decision endpoint.

### What "explainable" honestly means here

Showing the model's reasoning was requested citing responsible AI. The tempting reading is that a chain of thought explains the answer. It does not: measured faithfulness sits at roughly 25 to 39%, and the BIS Financial Stability Institute has put on record that reasoning models' explanations "do not necessarily reflect how the models actually arrive at their conclusions". NIST IR 8312 requires an explanation to reflect the system's actual process.

So the panel shows two things and keeps them apart. The **steps** are a record: each is a call the gateway made, named by the tool manifest rather than by a model, so it says what happened and says the same thing every run. The **model's text** sits below them, labelled as working text, with a line stating plainly that it is not a record of how the answer was produced and not a credit assessment.

Two placement decisions follow from the same evidence. It sits **below** the answer, not above, because detailed explanations are known to increase reliance on a recommendation including when the recommendation is wrong, and a preamble frames an answer before it is read. And it is **closed by default including on write turns**, because expanding a plausible-but-unfaithful narrative at the moment an officer is deciding is the worst possible placement.

Four locales had translated the heading as *Reasoning*. That is the word this panel deliberately avoids, and it took a review comment to notice that the English decision had not survived translation.
...
Almost every pull request after the gateway landed exists because someone used the thing for real and it fell short. Writes failed every evening because commands were dated from the server's clock rather than Fineract's business date. A client was created with a fifteen-digit first name. A lowercase search reported a client did not exist. Two quick questions produced "the model is unavailable" when the officer was rate limited for seven seconds.

The lesson I would carry into any project: a demo proves a feature exists, and only sustained use by somebody who did not build it proves the feature works.
...
To **Victor Romero**, for reviewing like a banker rather than like a linter. He never handed me the fix. He would say something like "remember that in production the API URL can be different from the UI URL because of an API gateway" and leave me to work out what that meant for the design. Learning to read a review comment as a question about users, rather than a complaint about code, is the most useful thing I take from this summer.

To the **Mifos community** for the sandbox, the answers and the patience, and to **Google Summer of Code** for the opportunity.

## Future Work

- **State that survives a restart.** Pending confirmation cards and conversations live in process memory. That is correct for one instance and wrong for a cluster, and it is the first thing a production deployment will need.
- **Group and centre lending.** The manifest covers individual lending only, which leaves out a great deal of how microfinance actually works. It is the obvious next surface, and the manifest is designed so adding it is a reviewable act rather than a prompt change.
- **On-premise inference, documented.** Running the model through Ollama already works and has been tested against a live instance, but it is not yet written up for institutions that cannot send data to a cloud provider. That audience is exactly the one this matters most for.
- **A permalink for a reply.** Deliberately absent today: conversations are held in the browser so client details never accumulate on the gateway, and a shareable link would need the opposite. Worth revisiting only with the data-residency question answered first.
...
- [Copilot Gateway documentation](https://github.com/openMF/mcp-mifosx/blob/main/gateway/README.md) — tools, the confirmation flow, and how to run and deploy it
